...
public class CustomSteps {
    ...
    /**
     * Step Definition to verify all subtabs under a main tab are present and that the current subtab is highlighted.
     * Subtab names are retrieved dynamically from Excel using test data placeholder references.
     * 
     * Example Gherkin usage:
     * Then I verify subtabs under "FunctionalTabBlock" on page "EntityDetails" using test data key "ExpectedSubtabs" and highlighted subtab is "Entity Information"
     * 
     * @param mainTabName     Logical name of the main tab block (OR reference)
     * @param pageName        Logical page name from OR
     * @param expectedTabKey  Excel key (e.g., EntityDetails.ExpectedSubtabs) holding comma-separated expected subtab names
     * @param expectedHighlight Expected name of the subtab that should be visually selected/highlighted
     * @throws Throwable if verification fails
     */
    @Then("I verify subtabs under {string} on page {string} using test data key {string} and highlighted subtab is {string}")
    public void verifySubtabsUnderMainTab(String mainTabName, String pageName, String expectedTabKey, String expectedHighlight) throws Throwable {
        try {
            logger.info("Verifying subtabs under '{}' on page '{}' using key '{}' with highlight '{}'", mainTabName, pageName, expectedTabKey, expectedHighlight);

            // Resolve test data
            String resolvedTabNames = commonSteps.replaceTestDataPlaceholders(expectedTabKey);
            expectedHighlight = commonSteps.replaceTestDataPlaceholders(expectedHighlight);

            List<String> expectedSubtabs = Arrays.stream(resolvedTabNames.split(","))
                                                 .map(String::trim)
                                                 .collect(Collectors.toList());

            WebElement mainTabContainer = commonSteps.findElement(mainTabName, pageName);
            if (mainTabContainer == null) {
                throw new RuntimeException("Main tab container not found: " + mainTabName);
            }

            // Assume subtabs are <div role='tab'> or <button role='tab'> or similar inside the container
            List<WebElement> subtabElements = mainTabContainer.findElements(By.cssSelector("[role='tab']"));

            List<String> actualSubtabTexts = new ArrayList<>();
            String actualHighlightedTab = null;

            for (WebElement tab : subtabElements) {
                String text = tab.getText().trim();
                actualSubtabTexts.add(text);

                // If this subtab is marked active (e.g. aria-selected or class)
                String selected = tab.getAttribute("aria-selected");
                if ((selected != null && selected.equalsIgnoreCase("true")) ||
                    tab.getAttribute("class").toLowerCase().contains("active") ||
                    tab.getCssValue("color").equalsIgnoreCase("rgba(22, 119, 255, 1)")) {
                    actualHighlightedTab = text;
                }
            }

            // Compare subtabs
            if (!expectedSubtabs.equals(actualSubtabTexts)) {
                logger.error("Mismatch in subtabs: Expected: {}, Found: {}", expectedSubtabs, actualSubtabTexts);
                ReportManager.logFail("Verify Subtabs", "Expected: " + expectedSubtabs + ", Found: " + actualSubtabTexts);
                commonSteps.takeScreenshot("SubtabMismatch_" + mainTabName);
                throw new AssertionError("Subtab mismatch under: " + mainTabName);
            }

            // Verify highlighted tab
            if (!expectedHighlight.trim().equalsIgnoreCase(actualHighlightedTab)) {
                logger.error("Highlight mismatch: Expected '{}', Found '{}'", expectedHighlight, actualHighlightedTab);
                ReportManager.logFail("Highlighted Subtab", "Expected: " + expectedHighlight + ", Found: " + actualHighlightedTab);
                commonSteps.takeScreenshot("SubtabHighlightMismatch_" + mainTabName);
                throw new AssertionError("Highlighted tab mismatch under: " + mainTabName);
            }

            ReportManager.logPass("Subtabs under '" + mainTabName + "'", "All expected subtabs matched and highlight is correct.");
            logger.info("Subtabs verified successfully: {}", actualSubtabTexts);

        } catch (Exception e) {
            logger.error("Error verifying subtabs under '{}'", mainTabName, e);
            ReportManager.logFail("Verify Subtabs under '" + mainTabName + "'", e.getMessage());
            commonSteps.takeScreenshot("VerifySubtabs_Exception_" + mainTabName);
            throw new RuntimeException("Error verifying subtabs: " + e.getMessage(), e);
        }
    }
    ...
    /**
     * Step Definition to verify if a field highlights with a violet border on both hover and focus (click or enter).
     * Accepts a list of equivalent violet RGB values that may vary slightly across themes.
     *
     * Example Gherkin:
     * Then I verify the field "FirstName" on page "EntityListingPage" highlights with violet on hover
     */
    @Then("I verify the field {string} on page {string} highlights with violet on hover")
    public void verifyFieldVioletHoverHighlight(String fieldName, String pageName) throws Exception {
        try {
            logger.info("Verifying violet hover and focus border for field '{}' on page '{}'", fieldName, pageName);

            fieldName = commonSteps.replaceTestDataPlaceholders(fieldName);
            pageName = commonSteps.replaceTestDataPlaceholders(pageName);

            WebElement field = commonSteps.findElement(fieldName, pageName);
            if (field == null) throw new RuntimeException("Element not found: " + pageName + "." + fieldName);

            // List of accepted violet RGB values
            List<String> acceptedVioletShades = Arrays.asList(
                "rgb(81, 69, 158)",
                "rgba(81, 69, 158, 1)"
            );

            // Hover over the field
            new Actions(driver).moveToElement(field).pause(Duration.ofMillis(500)).perform();
            String hoverBorderColor = field.getCssValue("border-color").trim();
            logger.info("Hover border-color: {}", hoverBorderColor);

            if (!acceptedVioletShades.contains(hoverBorderColor)) {
                String msg = "Field '" + fieldName + "' did not highlight with violet on hover. Found: " + hoverBorderColor;
                ReportManager.logFail("Hover highlight validation", msg);
                throw new Exception(msg);
            } else {
                ReportManager.logPass("Hover highlight validation",
                    "Field '" + fieldName + "' highlights with violet on hover.");
            }

            // Click (focus) the field
            field.click();
            Thread.sleep(300); // slight delay for focus effect
            String focusBorderColor = field.getCssValue("border-color").trim();
            logger.info("Focus (click) border-color: {}", focusBorderColor);

            if (!acceptedVioletShades.contains(focusBorderColor)) {
                String msg = "Field '" + fieldName + "' did not highlight with violet on focus. Found: " + focusBorderColor;
                ReportManager.logFail("Focus highlight validation", msg);
                throw new Exception(msg);
            } else {
                ReportManager.logPass("Focus highlight validation",
                    "Field '" + fieldName + "' highlights with violet on focus.");
            }

        } catch (Exception e) {
            String errorMessage = "Error verifying hover/focus effect for field '" + fieldName + "' on page '" + pageName + "': " + e.getMessage();
            throw new Exception(errorMessage, e);
        }
    }
    
    /**
     * Step Definition: Verifies that a specific row (with expected values) is present in a grid.
     *
     * This method:
     * - Parses expected cell values from the Gherkin step.
     * - Locates the corresponding row using a locator key from the OR JSON file.
     * - Compares actual cell values in the row with expected values.
     * - Logs mismatches and throws an error if validation fails.
     * - Captures a screenshot and logs results via ReportManager.
     *
     * @param expectedValuesString A comma-separated string of expected values (e.g., "[One, Two, Three]").
     * @param pageName             The OR JSON page name where the locator key exists.
     * @param jsonKey              The key used to locate the row element in the OR.
     */
    @Then("Verify the saved row {string} is present in the grid on {string} using key {string}")
    public void verifyGridRowUsingKey(String expectedValuesString, String pageName, String jsonKey) throws IllegalArgumentException {
        logger.info("=== STARTING GRID ROW VERIFICATION ===");

        try {
            // Parse expected values from input string
            List<String> expectedValues = Arrays.stream(expectedValuesString.replace("[", "").replace("]", "").split(","))
            								.map(String::trim).filter(s -> !s.isEmpty()).collect(Collectors.toList());

            logger.info("Expected values to verify: {}", expectedValues);
            logger.info("Verifying grid row using page: '{}', key: '{}'", pageName, jsonKey);

            if (pageName == null || pageName.trim().isEmpty()) {
                throw new IllegalArgumentException("Page name is null or empty. Cannot fetch locator.");
            }

            if (jsonKey == null || jsonKey.trim().isEmpty()) {
                throw new IllegalArgumentException("JSON key is null or empty. Cannot fetch locator.");
            }

            // Load locator from OR using provided key
            By rowLocator = ORLoader.getLocator(pageName, jsonKey);
            if (rowLocator == null) {
                throw new RuntimeException("No locator found for key: " + jsonKey + " in page: " + pageName);
            }

            // Wait until the row is visible
            long waitTime = Long.parseLong(configLoader.getConfigValue("Explicit_Wait"));
            WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(waitTime));
            WebElement rowElem = wait.until(ExpectedConditions.visibilityOfElementLocated(rowLocator));
            logger.info("Row element located successfully");

            // Extract cells from the row
            List<WebElement> cells = rowElem.findElements(By.xpath("./td | ./th"));
            logger.info("Found {} cells in the row", cells.size());

            if (cells.isEmpty()) {
                throw new AssertionError("No cells found in the located row");
            }

            if (expectedValues.size() != cells.size()) {
                String errorMsg = String.format("Column count mismatch - Expected: %d columns, Found: %d columns. Expected values: %s",
                                                expectedValues.size(), cells.size(), expectedValues);
                logger.error(errorMsg);
                throw new AssertionError(errorMsg);
            }

            // Compare actual vs expected cell values
            List<String> mismatches   = new ArrayList<>();
            List<String> actualValues = new ArrayList<>();

            for (int i = 0; i < expectedValues.size(); i++) {
                String actual   = getCellText(cells.get(i));
                String expected = expectedValues.get(i);
                actualValues.add(actual);

                logger.info("Column {} - Expected: '{}', Actual: '{}'", i + 1, expected, actual);

                if (!expected.equals(actual)) {
                    mismatches.add(String.format("Column %d: Expected '%s' but found '%s'", i + 1, expected, actual));
                }
            }

            if (!mismatches.isEmpty()) {
                String error = String.format("Grid row verification failed:\nExpected: %s\nActual: %s\nMismatches:\n%s",
                                             expectedValues, actualValues, String.join("\n", mismatches));
                logger.error(error);
                addDemoDelay();
                try {
                    commonSteps.takeScreenshot("Grid Row Verification Failed - Mismatches Found");
                } catch (Throwable t) {
                    logger.error("Error taking screenshot: {}", t.getMessage());
                    throw new RuntimeException("Screenshot failed: " + t.getMessage(), t);
                }
            ...
            logger.info("All values matched successfully. Expected: {}, Actual: {}", expectedValues, actualValues);
            addDemoDelay();
            try {
                commonSteps.takeScreenshot("Grid Row Verified Successfully - " + expectedValues);
            } catch (Throwable t) {
                logger.error("Error taking screenshot: {}", t.getMessage());
                throw new RuntimeException("Screenshot failed: " + t.getMessage(), t);
            }

            ReportManager.logPass("Grid Row Verification", "Successfully verified row with values: " + expectedValues);

        } catch (RuntimeException e) {
            logger.error("Grid row verification failed with configuration/setup error: {}", e.getMessage(), e);
            addDemoDelay();
            try {
                commonSteps.takeScreenshot("Grid Row Verification Failed - Setup Error");
            } catch (Throwable t) {
                logger.error("Error taking screenshot: {}", t.getMessage());
                throw new RuntimeException("Screenshot failed: " + t.getMessage(), t);
            }
            ReportManager.logFail("Grid Row Verification", "Setup/Configuration Error: " + e.getMessage());
            throw e;

        } catch (AssertionError e) {
            logger.error("Grid row verification failed with assertion error: {}", e.getMessage());
            ReportManager.logFail("Grid Row Verification", "Assertion Failed: " + e.getMessage());
            throw e;

        } catch (Exception e) {
            logger.error("Grid row verification failed with unexpected error: {}", e.getMessage(), e);
            addDemoDelay();
            try {
                commonSteps.takeScreenshot("Grid Row Verification Failed - Unexpected Error");
            } catch (Throwable t) {
                logger.error("Error taking screenshot: {}", t.getMessage());
                throw new RuntimeException("Screenshot failed: " + t.getMessage(), t);
            }
            ReportManager.logFail("Grid Row Verification", "Unexpected Error: " + e.getMessage());
            throw new RuntimeException("Grid row verification failed due to unexpected error", e);
        }

        logger.info("=== GRID ROW VERIFICATION COMPLETED SUCCESSFULLY ===");
    }
    ...
    /**
     * Helper method to extract text from a cell, handling various element types
     * and potential null/empty scenarios.
     * 
     * @param cell The WebElement representing the table cell
     * @return The text content of the cell, trimmed and normalized
     */
    private String getCellText(WebElement cell) {
        ...
        try {
            ...
            if (text == null || text.trim().isEmpty()) {
                ...
                if (text == null) {
                    text = cell.getAttribute("textContent");
                }
            }
            return text != null ? text.trim() : "";

        } catch (Exception e) {
            logger.warn("Failed to extract text from cell: {}", e.getMessage());
            return "";
        }
    }
